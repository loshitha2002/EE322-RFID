// ============================================================================
// main_final_real_lock_PC3_LOCK.asm
// Based on: main.asm (known-good mid-evaluation firmware)
// Goal: final-hardware friendly version WITHOUT editing main.asm
// Changes:
// - Configurable lock output polarity (active-high/active-low)
// - Configurable unlock time in seconds
// - PCB variant: LOCK output moved to PC3 (A3). LED moved to PD7 (D7).
// ============================================================================
...
// ============================================================================
// FINAL HARDWARE OPTIONS (edit only these for your real lock)
// ============================================================================
// If your driver turns ON when LOCK pin is HIGH (typical MOSFET low-side): set 1
// If your driver turns ON when LOCK pin is LOW (some relay modules): set 0
.equ LOCK_ACTIVE_HIGH = 1

// How long to keep the lock open after ACCESS GRANTED (seconds)
.equ UNLOCK_TIME_S = 3

// ============================================================================
// Macros: lock control (keeps behavior consistent everywhere)
// ============================================================================
.macro LOCK_ON
    .if LOCK_ACTIVE_HIGH
        sbi     PORTC, PC3
...
RESET:
    ...
    ; pinMode(PIN_LOCK, OUTPUT)    -> PC3 output
    ; pinMode(PIN_LED, OUTPUT)     -> PD7 output
    sbi     DDRB, PB0              ; Buzzer = output
    sbi     DDRC, PC3              ; Lock = output
    sbi     DDRD, PD7              ; LED = output

    ; All outputs LOW initially
    ; digitalWrite(PIN_LOCK, LOW)
    ; digitalWrite(PIN_LED, LOW)
    cbi     PORTB, PB0             ; Buzzer OFF
    LOCK_OFF                        ; Lock OFF (polarity-safe)
    ...
    ldi     ZL, LOW(str_booting<<1)
    ldi     ZH, HIGH(str_booting<<1)
...
state_idle:
    ...
    ; lcd.print("Enter PIN:")
    ldi     ZL, LOW(str_enter_pin<<1)
    ldi     ZH, HIGH(str_enter_pin<<1)
    rcall   lcd_print_string
...
state_check_pin:
    ...
    ; lcd.print("Verifying...")
    ldi     ZL, LOW(str_verifying<<1)
    ldi     ZH, HIGH(str_verifying<<1)
    rcall   lcd_print_string
    ...
    ldi     ZL, LOW(secret_pin<<1)
    ldi     ZH, HIGH(secret_pin<<1)
...
check_pin_result:
    ...
    ; lcd.print("PIN OK!")
    ldi     ZL, LOW(str_pin_ok<<1)
    ldi     ZH, HIGH(str_pin_ok<<1)
    rcall   lcd_print_string
    ...
    ; lcd.print("Scan Card")
    ldi     ZL, LOW(str_scan_card<<1)
    ldi     ZH, HIGH(str_scan_card<<1)
    rcall   lcd_print_string
...
pin_wrong:
    ...
    ; lcd.print("Wrong PIN!")
    ldi     ZL, LOW(str_wrong_pin<<1)
    ldi     ZH, HIGH(str_wrong_pin<<1)
    rcall   lcd_print_string
    ...
    ldi     ZL, LOW(str_tries_left<<1)
    ldi     ZH, HIGH(str_tries_left<<1)
    rcall   lcd_print_string
...
pin_lockout:
    ...
    ldi     ZL, LOW(str_system_locked<<1)
    ldi     ZH, HIGH(str_system_locked<<1)
    rcall   lcd_print_string
...
state_wait_rfid:
    ...
    ldi     ZL, LOW(str_reading_card<<1)
    ldi     ZH, HIGH(str_reading_card<<1)
    rcall   lcd_print_string

    ; --- Compare scanned UID with SECRET_UID ---
    ldi     XL, LOW(scanned_uid)
    ldi     XH, HIGH(scanned_uid)
    ldi     ZL, LOW(secret_uid<<1)
    ldi     ZH, HIGH(secret_uid<<1)
...
rfid_unknown:
    ...
    ldi     ZL, LOW(str_unknown_card<<1)
    ldi     ZH, HIGH(str_unknown_card<<1)
    rcall   lcd_print_string
...
state_access_granted:
    ...
    ldi     ZL, LOW(str_access_granted<<1)
    ldi     ZH, HIGH(str_access_granted<<1)
    rcall   lcd_print_string
    ...
    ; delay(UNLOCK_TIME_S * 1000)
    ldi     r18, UNLOCK_TIME_S
unlock_wait_loop:
    rcall   delay_1000ms
    dec     r18
...
state_lockout:
    ...
    ldi     ZL, LOW(str_locked_out<<1)
    ldi     ZH, HIGH(str_locked_out<<1)
    rcall   lcd_print_string
    ...
    ; lcd.print("Wait: ")
    ldi     ZL, LOW(str_wait<<1)
    ldi     ZH, HIGH(str_wait<<1)
    rcall   lcd_print_string
    ...
    ; lcd.print("s")
    ldi     ZL, LOW(str_seconds<<1)
    ldi     ZH, HIGH(str_seconds<<1)
    rcall   lcd_print_string
...
lockout_finished:
    ...
    ldi     ZL, LOW(str_lock_released<<1)
    ldi     ZH, HIGH(str_lock_released<<1)
    rcall   lcd_print_string
...
reset_system:
    ...
    ; lcd.print("Tries: ")
    ldi     ZL, LOW(str_tries<<1)
    ldi     ZH, HIGH(str_tries<<1)
    rcall   lcd_print_string
    ...
    ; lcd.print(" left")
    ldi     ZL, LOW(str_left<<1)
    ldi     ZH, HIGH(str_left<<1)
    rcall   lcd_print_string
...
key_found:
    ...
    clr     r20
    adc     ZH, r20
    lpm     r24, Z                 ; r24 = key character (flash)
